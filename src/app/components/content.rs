...
impl AsyncComponent for ContentModel {
        ...
        gtk::Box {
            ...
            adw::TabView {
                append_pinned = &gtk::Box {
                    ...
                    set_icon: Some(&gio::ThemedIcon::new(icon_name::CAMERAS)),
                },
                ...
                append_pinned = &gtk::Box {
                    ...
                    set_icon: Some(&gio::ThemedIcon::new(icon_name::IMAGE)),
                },
            },
        }
    }
    ...
    async fn init(
        ...
        let video_list_controller = VideoListModel::builder()
            .launch(())
        ...
        let image_list_controller = ImageListModel::builder()
            .launch(())
            .forward(sender.input_sender(), |output| match output {
                ImageListOutput::VideoConversionCompleted => ContentInput::VideoConversionCompleted,
    ...
    async fn update_with_view(
        ...
        match message {
            ...
            ContentInput::SearchCompleted(found) => {
                ...
                sender.output(ContentOutput::SearchCompleted(found)).unwrap_or_default();
            }
            ContentInput::FilterCount(count) => {
                sender.output(ContentOutput::FilterCount(count)).unwrap_or_default();
            }
            ContentInput::ConvertVideos(video_list, layout_type, dst_path) => {
                ...
                self.image_list.emit(ImageListInput::ConvertVideos(video_list, layout_type, dst_path));
            }
            ContentInput::VideoConversionCompleted => {
                widgets.tab_view.nth_page(IMAGE_PAGE).set_loading(false);
            }
            ContentInput::ImageCount(count) => {
                sender.output(ContentOutput::ImageCount(count)).unwrap_or_default();
            }
            ContentInput::Notify(msg, timeout) => {
                sender.output(ContentOutput::Notify(msg, timeout)).unwrap_or_default();
            }
       }
    }
}
